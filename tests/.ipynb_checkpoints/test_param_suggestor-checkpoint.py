...
class ModuleGlobalSuggestionTests(unittest.TestCase):
    ...
    def test_boolean_default_infers_type(self):
        ...
        sets_with_enabled = [ps for ps in suggestions.param_sets if "enabled" in ps]
        self.assertTrue(sets_with_enabled, "Expected a param set overriding enabled")
        self.assertEqual(sets_with_enabled[0]["enabled"], False)
    ...
    def test_param_history_used_as_fallback(self):
        ...
        dry_run_values = {ps["dry_run"] for ps in suggestions.param_sets if "dry_run" in ps}
        self.assertEqual(dry_run_values, {False})
