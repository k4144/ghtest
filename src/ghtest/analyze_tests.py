...
import os,time,re,sys,glob,yaml
...
def _get_test_files(test_dir, exclude=None, vb=0):
    ...
    for f in glob.glob(os.path.join(test_dir, '**', '*'), recursive=True):
        ...
        if os.path.isdir(f):
            continue
        if vb:print(os.path.basename(f).split('.')[0])
...
def _get_tests(test_modules):
    ...
    for f in test_modules:
        with open(f) as h:
            t=h.read()
        funcs=[l[9:].rstrip(':()') for l in t.splitlines() if l.startswith('def test')]
        n=_get_base_test_name(f)
...
def _load_test_data(test_objects_dir='testdata_test_objects', vb=0):
    ...
    for p in objs:
        ...
        if not os.path.isfile(path):
            if vb:print(f'{path} not found')
            continue
        with open(path, 'rb')as f:
            ret.append(dill.load(f))
    if not ret:
...
def print_scan(scs):
    for sc in scs:
        m,n,ps=sc.module, sc.qualname, sc.parameters
...
def print_suggestion(sps):
    for sc in sps:
        m,n,ps=sc.module, sc.qualname, sc.param_sets
        print()
        print(n)
        for p in ps:
            print(p)
...
def print_test_summary(cassette_dir = 'testdata_tests/cassettes' ,test_dir = 'testdata_tests',src_dir = 'testdata', data_dir='data', 
                       test_objects_dir='testdata_test_objects', refresh=False, vb=0):
    ...
    if refresh:
        try:
            ...
            print(f'test objects not found, abandoning detailed statistics printout')
            return
    else:
        try:
            scs, sps, gts, trs = _load_test_data(test_objects_dir=test_objects_dir)
        except Exception:
            print(f'error reading test objects, abandoning detailed statistics printout')
            return
    if scs:
        _print_line('scans')
        print_scan(scs)
...
if __name__=='__main__':
    ...
    data_dir='data'
    vb=2
    print_test_summary(cassette_dir = 'testdata_tests/cassettes', test_dir = 'testdata_tests', src_dir = 'testdata', data_dir='data', vb=vb)
