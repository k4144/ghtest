...
def create_tests(
    ...
    scs = [
        f
        for f in scs
        if not f.qualname.startswith("_")
        and not f.qualname.startswith("test_")
        and not f.qualname.startswith("scenario_")
        and f.qualname != "main"
    ]
    if vb > 1:
    ...
    for sp, tr in zip(sps_targeted if "sps_targeted" in locals() else sps, trs):
        if tr is not None:
            artifacts.append(TestArtifact(suggestion=sp, run=tr))  # noqa: E701
